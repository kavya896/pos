...
//       const length = await Item.find({}).countDocuments() 
//       const skip = (page - 1) * limit

//       const ittems = await Item.find({}).sort({_id: -1}) 
//       .skip(skip).limit(parseInt(limit))
//       if (ittems.length > 0) {
//           res.status(200).send({success:true, ittems})
//       } else {
//           res.status(400).send({success:false, message: "list is empty" })
//       }
//   } catch (err) {
...
exports.Item = async (req, res) => {
  try {
      console.log("entered")
      const { name,categoryName,typeOfFood,options, description, image, available, soldBy, price, cost, sku, composite, inStock, lowStock, variants, spiceLevel,tax } = req.body
      ...
      if (exists.length > 0) {
          ...
          const updateCategory = await Category.find({ name: categoryName })
          if (updateCategory.length > 0) {
              updateCategory[0].noOfItems = updateCategory[0].noOfItems + 1
              await updateCategory[0].save()

          
          const item = await Item.create({ name,categoryName, category:updateCategory[0]._id, typeOfFood,options, image, description, available, soldBy, price, cost, sku, composite, inStock, lowStock,variants, spiceLevel, tax })
          await item.save()
          console.log(item)
          res.status(200).send(item)
          }
