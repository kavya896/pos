...
exports.order = async (req, res) => {
  try {
    ...
    let deleteQuery;
    if (cartData._id) {
      deleteQuery = { _id: cartData._id };
    } else if (cartData.cartId) {
      deleteQuery = { _id: cartData.cartId };
    } else {
      console.error("Neither _id nor cartId is present");
    }
    ...
    if (cart.odrderId) {
      ...
      // createtransactionOnPayment(order)
    }
  } catch (error) {
    console.log(error);
    res.status(500).send({
...
// const createtransactionOnPayment = async(order)=>{
//   const transaction = await Transaction.create({
//     orderNo:order._id,
//     amount:order.grandTotal,
//     totalNoOfItems:order.item.length,
...
exports.billing = async(req,res)=>{
  try {
    const { cartData, discount, grandTotal, orderStatus, paymentStatus, payment} = req.body
    await Orders.updateOne({_id:cartData._id},{
        $set: {
          discount: discount,
          grandTotal: grandTotal,
          orderStatus: orderStatus,
  ...
  try {
    ...
    if (cart) {
      if (cart.items.length > 0) {
        res
          .status(400)
          .send({ success: false, message: "Please clear the cart" });
      } else {
        ...
        if (removeCart) {
          ...
          res
            .status(200)
            .send({ success: true, orderCart, message: "edit cart" });
        }
      }
    } else {
      const orderCart = await Orders.findOne({ cartId: cartId });
...
exports.getPaidOrders = async(req,res)=>{
  try {
    ...
    const paidOrders = await Orders.find({ 
      ...
      .sort({ updatedAt: -1 })
      .populate("item.product")
      .populate("employeeId").populate("customerId")
       res.status(200).send({
      success: true,
      paidOrders,
    });
